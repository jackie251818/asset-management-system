...
| 任何时候进入左侧菜单 **系统设置** 页（v3.7.2+，版本信息下方直接可见的「检查更新」按钮）；v3.7.1 旧版则按 **Alt 键** → 设置 → **检查更新** | 手动检查："已是最新 / 发现新版本 / 无法获取更新信息 / 单机模式不支持"均有明确提示（v3.7.2+ 在页面内展示，不再弹系统对话框） |
...
- 直接重启客户端：约 6 秒后应弹「发现新版本 x.y.z」；或进入 **系统设置** 页点版本信息下方的「检查更新」（v3.7.1 旧版为 Alt → 设置 → 检查更新）
- 点「立即更新」，观察页面内下载进度条 → 应用自动重启
- 重启后再点「检查更新」应提示"当前已是最新版本 x.y.z"
...
| 2026-09-13 | 3.7.2 | `3017d0ca4135` | 系统设置页新增直接可见的「检查更新」按钮（解决 Alt 菜单无法调出的问题）；更新确认弹窗/下载进度改为应用内页面展示（新增 IPC `update:check`/`update:apply` + `update:progress`/`update:error` 事件） | 开发机构建部署，服务器 sha256 一致 + HTTP 验证通过；待客户端实测 |
